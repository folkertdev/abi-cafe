...
impl TestHarness {
    ...
    ///
    /// A test where both sides are c can be linked by a c compiler, and then the
    /// target doesn't need a rust stdlib at all. Anything else goes through the
    /// *plain* rustc, and not either side's rustc, which may have a custom
    /// codegen backend we don't want the harness built with.
